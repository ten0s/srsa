...
public class ArrayLocalMinimum {
    ...
    private static int indexOf(int[] a, int lo, int hi) {
        ...
        if (diff == 1) {
            ...
            else if (a[lo] > a[hi]) return hi;
            else throw new IllegalArgumentException("Some not distinct values");
        }
        int mid = lo + (hi - lo) / 2;
        if      (a[mid-1] < a[mid])   return indexOf(a, lo, mid-1);
        else if (a[mid]   > a[mid+1]) return indexOf(a, mid+1, hi);
        else                          return mid;
    }
    //+END_SOLUTION
